 ...
 * Env: MCP_PORT (default 8979), MCP_HOST (default 0.0.0.0); catalog DB: CATALOG_DB + DATA_DIR (required)
 */
...
// The catalog DB opens BEFORE anything binds: missing files / DDL failure = boot failure, exit 1.
try {
  await openCatalogDb(configFromEnv());
} catch (e: any) {
  console.error(`[db] boot failed: ${e?.message ?? e}`);
  process.exit(1);
}
...
app.get("/health", async (_req, res) => {
  res.json({ status: "ok", catalog: await stats() });
});
...
// Stateless transport does not use GET (SSE) or DELETE-session requests.
...
app.listen(PORT, HOST, async () => {
  ...
  try { const g = await ensureGrainRegistry(); console.error(`[grain] registry ${g.built ? "built" : "present"}: ${g.rows} multi-row tables`); }
  ...
  try { const u = await ensureUsageGraph(); console.error(`[usage] graph ${u.built ? "built" : "present"}: ${u.rows} usages over ${u.tables} tables`); }
  ...
  try { const p = await ensurePredicateRegistry(); console.error(`[predicates] rollup ${p.built ? "built" : "present"}: ${p.rows} predicates over ${p.tables} tables`); }
