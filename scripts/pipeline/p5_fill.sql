...
--
-- Run AFTER p5_ddl.sh, which created {{V}} (substituted by run_sql.sh) from the product's own
...
INSERT INTO {{V}}.report_queries_vec_multi (id, qrowid, embedding)
SELECT row_number() OVER (ORDER BY rq.rid, v.slot), rq.rid, v.embedding
FROM   work.embeddings v
JOIN   {{V}}.report_queries rq ON rq.id = 'sql:' || v.owner_id
...
INSERT INTO {{V}}.facts_meta (k, v) VALUES ('version', '1');
INSERT INTO {{V}}.facts_meta (k, v)
...
-- ===========================================================================
INSERT INTO {{V}}.tables (name, schema, type, module, remarks, view_text)
...
INSERT INTO {{V}}.columns (table_name, name, data_type, size, nullable, remarks, ordinal)
...
WHERE  work.nn(c.column_name) IS NOT NULL
  AND  EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = work.nn(c.table_name));

INSERT INTO {{V}}.pkeys (table_name, column_name, seq)
...
WHERE  work.nn(p.column_name) IS NOT NULL
  AND  EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = work.nn(p.table_name));
...
INSERT INTO {{V}}.fkeys (child_table, parent_table, column_name, seq, name)
...
WHERE  work.nn(f."table") IS NOT NULL AND work.nn(f.foreign_table) IS NOT NULL
  AND (EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = work.nn(f."table"))
    OR EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = work.nn(f.foreign_table)));

INSERT INTO {{V}}.indexes (table_name, index_name, is_unique, ordinal, column_name)
...
WHERE  EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = work.nn(i.table_name));

INSERT INTO {{V}}.relationships
...
INSERT INTO {{V}}.table_usages (table_name, query_id, source, title, sql_chars, score)
...
FROM (
  ...
  JOIN   {{V}}.report_queries rq ON rq.id = 'sql:' || ft.sql_hash
  ...
  WHERE  EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = ft.table_name)) x
WHERE  rn <= 60;
INSERT INTO {{V}}.usage_meta (k, v) VALUES ('version', '1');
...
INSERT INTO {{V}}.table_predicates (table_name, column_name, op, literal, occurrences, role)
...
JOIN  (SELECT table_name, column_name, count(DISTINCT left(literal, 80)) AS distinct_literals
       ...
       WHERE  EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = q.table_name)
...
WHERE  EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = p.table_name)
...
INSERT INTO {{V}}.pred_meta (k, v) VALUES ('version', '1');
...
INSERT INTO {{V}}.table_join_columns (table_name, column_name, units, share)
...
WHERE  j.units::numeric / d.units >= 0.20
  ...
  AND  EXISTS (SELECT 1 FROM {{V}}.tables t WHERE t.name = j.t);
...
INSERT INTO {{V}}.table_grain (table_name, grain, multi_row, dedup, signals, corpus_evidence, note, updated_at)
SELECT s.table_name,
...
INSERT INTO {{V}}.grain_meta (k, v) VALUES ('version', '4');

-- ===========================================================================
-- 5. the curated layout-pattern corpus (46 git-versioned rows — an INPUT)
-- ===========================================================================
INSERT INTO {{V}}.layout_patterns
...
INSERT INTO {{V}}.layout_patterns_vec (id, prowid, embedding)
...
INSERT INTO {{V}}.col_vec (hash, vec) SELECT hash, vec FROM v2026_09.col_vec;
...
-- ===========================================================================
INSERT INTO {{V}}.catalog_meta (key, value) VALUES
  ('schema_version',    '2'),
  ('queries_version',   '2'),
  ('embedding_version', 'bge-small-en-v1.5-384'),
  ('ddl_version',       (SELECT v FROM work.build_meta WHERE k = 'ddl_version')),
  ('tables',            (SELECT count(*)::text FROM {{V}}.tables)),
  ('columns',           (SELECT count(*)::text FROM {{V}}.columns)),
  ('fkeys',             (SELECT count(*)::text FROM {{V}}.fkeys)),
  ('relationships',     (SELECT count(*)::text FROM {{V}}.relationships)),
  ('report_queries',    (SELECT count(*)::text FROM {{V}}.report_queries)),
  ('built_at_epoch',    (extract(epoch FROM now()) * 1000)::bigint::text);

-- identity sequences past the explicit keys we inserted
SELECT setval(pg_get_serial_sequence('{{V}}.report_queries', 'rid'),
              coalesce((SELECT max(rid) FROM {{V}}.report_queries), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('{{V}}.report_queries_vec_multi', 'id'),
              coalesce((SELECT max(id) FROM {{V}}.report_queries_vec_multi), 0) + 1, false);
SELECT setval(pg_get_serial_sequence('{{V}}.table_rules', 'id'), 1, false);

ANALYZE {{V}}.report_queries;
ANALYZE {{V}}.report_queries_vec_multi;
ANALYZE {{V}}.tables;
ANALYZE {{V}}.columns;
ANALYZE {{V}}.table_usages;
ANALYZE {{V}}.table_predicates;
ANALYZE {{V}}.table_grain;
